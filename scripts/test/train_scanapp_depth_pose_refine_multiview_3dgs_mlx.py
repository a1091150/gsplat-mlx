...
class ScanAppPoseResiduals(nn.Module):
    @classmethod
    def zeros(cls, frame_count: int) -> "ScanAppPoseResiduals":
        model = cls.__new__(cls)
        nn.Module.__init__(model)
        model.rot = mx.zeros((1, frame_count, 3), dtype=mx.float32)
        model.trans = mx.zeros((1, frame_count, 3), dtype=mx.float32)
        return model
...
def pose_delta_viewmats(rot_deltas: mx.array, trans_deltas: mx.array, batch_ids: list[int]) -> mx.array:
    batch_indices = mx.array(batch_ids, dtype=mx.int32)
    rot = mx.take(rot_deltas, batch_indices, axis=1)
    trans = mx.take(trans_deltas, batch_indices, axis=1)
    rx = rot[..., 0]
    ry = rot[..., 1]
    rz = rot[..., 2]
    tx = trans[..., 0]
    ty = trans[..., 1]
    tz = trans[..., 2]
    zeros = mx.zeros_like(rx)
    ones = mx.ones_like(rx)
    return mx.stack(
        [
            mx.stack([ones, -rz, ry, tx], axis=-1),
            mx.stack([rz, ones, -rx, ty], axis=-1),
            mx.stack([-ry, rx, ones, tz], axis=-1),
            mx.stack([zeros, zeros, zeros, ones], axis=-1),
        ],
        axis=-2,
    )


def refined_viewmats_from_pose_residuals(
    base_viewmats: mx.array,
    rot_deltas: mx.array,
    trans_deltas: mx.array,
    batch_ids: list[int],
) -> tuple[mx.array, mx.array, mx.array]:
    delta_viewmats = pose_delta_viewmats(rot_deltas, trans_deltas, batch_ids)
    return delta_viewmats @ base_viewmats, mx.take(rot_deltas, mx.array(batch_ids, dtype=mx.int32), axis=1), mx.take(
        trans_deltas,
...
def apply_pose_residuals_to_cameras(
    cameras: list[ScanAppCamera],
    rot_deltas: np.ndarray,
    trans_deltas: np.ndarray,
) -> list[ScanAppCamera]:
    refined = []
    for index, camera in enumerate(cameras):
        rx, ry, rz = rot_deltas[index].astype(np.float32)
        tx, ty, tz = trans_deltas[index].astype(np.float32)
        delta = np.array(
            [
                [1.0, -rz, ry, tx],
                [rz, 1.0, -rx, ty],
        ...
        refined.append(
            ScanAppCamera(
                index=camera.index,
                viewmat=(delta @ camera.viewmat).astype(np.float32),
                K=camera.K,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/scanapp_depth_pose_refine_train"))
    ...
    parser.add_argument("--pose-refine-enabled", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--pose-refine-stop-step", type=int, default=3000)
    parser.add_argument("--pose-rot-lr", type=float, default=1.0e-4)
    parser.add_argument("--pose-trans-lr", type=float, default=1.0e-3)
    parser.add_argument("--pose-rot-reg", type=float, default=1.0e-2)
    parser.add_argument("--pose-trans-reg", type=float, default=1.0e-2)
...
def main() -> None:
    ...
    if args.pose_refine_stop_step < 0:
        raise ValueError("--pose-refine-stop-step must be nonnegative")
    if args.pose_rot_reg < 0.0 or args.pose_trans_reg < 0.0:
        raise ValueError("--pose regularization weights must be nonnegative")
    ...
    for name, value in [
        ...
        ("--pose-rot-lr", args.pose_rot_lr),
        ("--pose-trans-lr", args.pose_trans_lr),
    ...
    pose_residuals = ScanAppPoseResiduals.zeros(len(cameras))
    ...
    def sh_loss_fn(
        ...
        refined_viewmats, batch_rot_deltas, batch_trans_deltas = refined_viewmats_from_pose_residuals(
            viewmats,
            rot_deltas,
            trans_deltas,
            batch_ids,
        ...
        losses = []
        radii = []
        batch = int(refined_viewmats.shape[1])
        for idx in range(batch):
            render = render_sh_model(
                local,
                viewspace_points[:, idx : idx + 1],
                refined_viewmats[:, idx : idx + 1],
                ks[:, idx : idx + 1],
                width,
                height,
                args.tile_size,
        ...
        pose_loss = (
            float(args.pose_rot_reg) * mx.mean(batch_rot_deltas * batch_rot_deltas)
            + float(args.pose_trans_reg) * mx.mean(batch_trans_deltas * batch_trans_deltas)
            if args.pose_refine_enabled
            else mx.array(0.0, dtype=mx.float32)
        )
        return mx.mean(mx.stack(losses)) + pose_loss, mx.concatenate(radii, axis=1)

    grad_fn = mx.value_and_grad(sh_loss_fn, argnums=(0, 1, 2, 3, 4, 5, 6, 11, 12))
    optimizers = {
        ...
        "pose_rot": Adam(learning_rate=args.pose_rot_lr),
        "pose_trans": Adam(learning_rate=args.pose_trans_lr),
    }
    lr_schedules = {
        ...
        "pose_rot": make_lr_schedule(args.pose_rot_lr, None, 1.0, args.steps),
        "pose_trans": make_lr_schedule(args.pose_trans_lr, None, 1.0, args.steps),
    }
    ...
    for step in range(1, args.steps + 1):
        ...
        for name, schedule in lr_schedules.items():
            ...
            if name in ("pose_rot", "pose_trans") and (
                not args.pose_refine_enabled or (args.pose_refine_stop_step > 0 and step > args.pose_refine_stop_step)
            ):
                lr = 0.0
        ...
        if args.pose_refine_enabled and latest_lrs["pose_rot"] > 0.0:
            optimizers["pose_rot"].update(pose_residuals, {"rot": d_pose_rot})
            optimizers["pose_trans"].update(pose_residuals, {"trans": d_pose_trans})
        ...
        if step == 1 or step == args.steps or step % args.log_interval == 0:
            log(
                ...
                f"means_lr={latest_lrs['means']:.8g} pose_lr=({latest_lrs['pose_rot']:.3g},{latest_lrs['pose_trans']:.3g}) "
                f"viewspace_grad_norm={last_viewspace_grad_norm:.8f}"
            )
        if args.step_image_interval > 0 and step % args.step_image_interval == 0:
            step_image_count += 1
            step_refined_cameras = apply_pose_residuals_to_cameras(
                cameras,
                np.asarray(pose_residuals.rot[0], dtype=np.float32),
                np.asarray(pose_residuals.trans[0], dtype=np.float32),
            )
            image = render_step_grid(model, step_refined_cameras, width, height, args.tile_size, active_sh_degree)
    ...
    pose_rot_np = np.asarray(pose_residuals.rot[0], dtype=np.float32)
    pose_trans_np = np.asarray(pose_residuals.trans[0], dtype=np.float32)
    refined_cameras = apply_pose_residuals_to_cameras(cameras, pose_rot_np, pose_trans_np)
    pose_rot_norms = np.linalg.norm(pose_rot_np, axis=1)
    pose_trans_norms = np.linalg.norm(pose_trans_np, axis=1)
    pose_refinement_summary = {
        "enabled": bool(args.pose_refine_enabled),
        "stop_step": int(args.pose_refine_stop_step),
        "rot_lr": float(args.pose_rot_lr),
        "trans_lr": float(args.pose_trans_lr),
        "rot_reg": float(args.pose_rot_reg),
        "trans_reg": float(args.pose_trans_reg),
        "composition": "refined world-to-camera viewmat = small_delta_viewmat @ arkit_viewmat",
        "rot_norm_min": float(pose_rot_norms.min()) if pose_rot_norms.size else 0.0,
        "rot_norm_mean": float(pose_rot_norms.mean()) if pose_rot_norms.size else 0.0,
        "rot_norm_max": float(pose_rot_norms.max()) if pose_rot_norms.size else 0.0,
        "trans_norm_min": float(pose_trans_norms.min()) if pose_trans_norms.size else 0.0,
        "trans_norm_mean": float(pose_trans_norms.mean()) if pose_trans_norms.size else 0.0,
        "trans_norm_max": float(pose_trans_norms.max()) if pose_trans_norms.size else 0.0,
        "frame_residuals": [
            {
                "frame_index": int(camera.index),
                "rot": pose_rot_np[index].astype(float).tolist(),
                "trans": pose_trans_np[index].astype(float).tolist(),
                "rot_norm": float(pose_rot_norms[index]),
                "trans_norm": float(pose_trans_norms[index]),
            }
            for index, camera in enumerate(cameras)
        ],
    }
    ...
    final_stats = evaluate_frames(model, refined_cameras, targets, target_masks, width, height, args.tile_size, active_sh_degree, args.ssim_lambda, args.ssim_window_size)
    eval_final_stats = (
        evaluate_frames(model, eval_cameras, eval_targets, eval_masks, width, height, args.tile_size, active_sh_degree, args.ssim_lambda, args.ssim_window_size)
        if eval_cameras
        else []
    ...
    summary = {
        "dataset_type": "scanapp_depth_pose_refine_mobile_prior_masked",
        ...
        "pose_refinement": pose_refinement_summary,
        ...
        "initialization": {
            "type": "scanapp_depth_mobile_prior_reconstruction",
        ...
        "paper_mapping": {
            ...
            "LighthouseGS": {
                "implemented": [
                    ...
                    "learnable residual pose refinement over ScanApp ARKit camera poses",
                ],
                "not_implemented": [
    ...
    log(
        "ScanApp pose-refine mobile-prior masked depth multi-view training ok "
