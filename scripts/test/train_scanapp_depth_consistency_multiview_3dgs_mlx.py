...
def select_motion_quality_frames(frames: list[ScanAppFrame], min_motion_quality: float) -> tuple[list[ScanAppFrame], dict]:
    if min_motion_quality <= 0.0:
        return frames, {
            "enabled": False,
            "min_motion_quality": float(min_motion_quality),
            "input_frames": int(len(frames)),
            "selected_frames": int(len(frames)),
            "rejected_frames": 0,
            "selected_frame_indices": [int(frame.index) for frame in frames],
        }
    selected = [frame for frame in frames if frame.motion_quality is not None and frame.motion_quality >= min_motion_quality]
    if not selected:
        raise RuntimeError(f"No ScanApp frames passed --min-motion-quality={min_motion_quality}")
    values = [float(frame.motion_quality) for frame in frames if frame.motion_quality is not None]
    return selected, {
        "enabled": True,
        "min_motion_quality": float(min_motion_quality),
        "input_frames": int(len(frames)),
        "selected_frames": int(len(selected)),
        "rejected_frames": int(len(frames) - len(selected)),
        "motion_quality_min": float(min(values)) if values else None,
        "motion_quality_mean": float(np.mean(values)) if values else None,
        "motion_quality_max": float(max(values)) if values else None,
        "selected_frame_indices": [int(frame.index) for frame in selected],
    }


def shared_median_intrinsics(frames: list[ScanAppFrame]) -> np.ndarray:
    if not frames:
        raise RuntimeError("Cannot compute shared intrinsics without frames")
    stacked = np.stack([frame.intrinsics for frame in frames], axis=0).astype(np.float32)
    return np.median(stacked, axis=0).astype(np.float32)


def image_sharpness(path: Path, stride: int) -> float:
    ...
    if sample.shape[0] < 2 or sample.shape[1] < 2:
        return 0.0
    dx = np.abs(sample[:, 1:] - sample[:, :-1]).mean()
    dy = np.abs(sample[1:, :] - sample[:-1, :]).mean()
    return float(dx + dy)


def select_information_gated_keyframes(
    frames: list[ScanAppFrame],
    min_translation: float,
    window_size: int,
    sharpness_stride: int,
    min_frames: int,
) -> tuple[list[ScanAppFrame], dict]:
    if not frames:
        raise RuntimeError("No ScanApp frames selected")
    window_size = max(1, int(window_size))
    min_frames = max(1, int(min_frames))
    centers = {frame.index: scanner_camera_center(frame.camera_to_world) for frame in frames}
    sharpness = {frame.index: image_sharpness(frame.image_path, sharpness_stride) for frame in frames}
    selected = [frames[0]]
    candidate_window: list[ScanAppFrame] = []
    last_center = centers[frames[0].index]

    for frame in frames[1:]:
        distance = float(np.linalg.norm(centers[frame.index] - last_center))
        if distance < min_translation:
            continue
        candidate_window.append(frame)
        if len(candidate_window) >= window_size:
            best = max(candidate_window, key=lambda item: sharpness[item.index])
            selected.append(best)
            last_center = centers[best.index]
    ...
    fallback_used = False
    if len(selected) < min_frames and len(frames) > len(selected):
        fallback_used = True
        selected_indices = {frame.index for frame in selected}
        remaining = [frame for frame in frames if frame.index not in selected_indices]
        remaining.sort(key=lambda frame: sharpness[frame.index], reverse=True)
        selected.extend(remaining[: max(0, min_frames - len(selected))])
        selected.sort(key=lambda frame: (frame.index, frame.metadata_path.name))

    selected_set = {frame.index for frame in selected}
    stats = {
        "enabled": True,
        "input_frames": int(len(frames)),
        "selected_frames": int(len(selected)),
        "rejected_frames": int(len(frames) - len(selected_set)),
        "min_translation": float(min_translation),
        "window_size": int(window_size),
        "sharpness_stride": int(max(1, sharpness_stride)),
        "min_frames": int(min_frames),
        "fallback_used": bool(fallback_used),
        "sharpness_min": float(min(sharpness.values())) if sharpness else 0.0,
        "sharpness_mean": float(np.mean(list(sharpness.values()))) if sharpness else 0.0,
        "sharpness_max": float(max(sharpness.values())) if sharpness else 0.0,
        "selected_frame_indices": [int(frame.index) for frame in selected],
    }
    return selected, stats


def load_scanapp_cameras(
    frames: list[ScanAppFrame],
    width: int,
    height: int,
    shared_intrinsics: np.ndarray | None = None,
) -> list[ScanAppCamera]:
    cameras: list[ScanAppCamera] = []
    for frame in frames:
...
def depth_projection_arrays(frame: ScanAppFrame, shared_intrinsics: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray | None, np.ndarray]:
    depth = read_depth_float32(frame.depth_path, frame.depth_width, frame.depth_height)
    confidence = read_confidence(frame.confidence_path, frame.depth_width, frame.depth_height)
    k = (shared_intrinsics if shared_intrinsics is not None else frame.intrinsics).copy()
    k[0, :] *= float(frame.depth_width) / float(max(1, frame.width))
    k[1, :] *= float(frame.depth_height) / float(max(1, frame.height))
    return depth, confidence, k.astype(np.float32)


def project_points_to_depth_pixels(
    points: np.ndarray,
    camera: ScanAppCamera,
    depth_k: np.ndarray,
    depth_width: int,
    depth_height: int,
    ...
    z = camera_points[:, 2]
    valid_z = np.isfinite(z) & (z > MIN_DEPTH_METERS)
    u = depth_k[0, 0] * camera_points[:, 0] / np.maximum(z, 1.0e-8) + depth_k[0, 2]
    v = depth_k[1, 1] * camera_points[:, 1] / np.maximum(z, 1.0e-8) + depth_k[1, 2]
    x = np.rint(u).astype(np.int32)
    y = np.rint(v).astype(np.int32)
    in_bounds = valid_z & (x >= 0) & (x < depth_width) & (y >= 0) & (y < depth_height)
    return x, y, in_bounds


def depth_consistency_keep_mask(
    points: np.ndarray,
    source_frame_index: int,
    frames: list[ScanAppFrame],
    cameras: list[ScanAppCamera],
    depth_cache: list[tuple[np.ndarray, np.ndarray | None, np.ndarray]],
    neighbor_window: int,
    min_consistent_views: int,
    abs_depth_tol: float,
    rel_depth_tol: float,
    min_confidence: int,
    keep_unobserved: bool,
    ...
    start = max(0, source_frame_index - int(neighbor_window))
    end = min(len(frames), source_frame_index + int(neighbor_window) + 1)
    observed_counts = np.zeros((points.shape[0],), dtype=np.int16)
    consistent_counts = np.zeros((points.shape[0],), dtype=np.int16)
    projection_tests = 0
    for target_index in range(start, end):
        if target_index == source_frame_index:
            continue
        target_frame = frames[target_index]
        target_camera = cameras[target_index]
        target_depth, target_confidence, target_k = depth_cache[target_index]
        x, y, in_bounds = project_points_to_depth_pixels(points, target_camera, target_k, target_frame.depth_width, target_frame.depth_height)
        if not np.any(in_bounds):
            continue
        projected_depth = target_depth[y[in_bounds], x[in_bounds]]
        valid_depth = np.isfinite(projected_depth) & (projected_depth >= MIN_DEPTH_METERS) & (projected_depth <= MAX_DEPTH_METERS)
        if target_confidence is not None:
            valid_depth &= target_confidence[y[in_bounds], x[in_bounds]] >= min_confidence
        point_indices = np.nonzero(in_bounds)[0]
        observed_indices = point_indices[valid_depth]
        if observed_indices.size == 0:
            continue
        predicted_depth = (target_camera.viewmat @ np.concatenate(
            [points[observed_indices].astype(np.float32), np.ones((observed_indices.size, 1), dtype=np.float32)],
            axis=1,
        ).T).T[:, 2]
        measured_depth = projected_depth[valid_depth]
        tolerance = np.maximum(float(abs_depth_tol), float(rel_depth_tol) * measured_depth)
        consistent = np.abs(predicted_depth - measured_depth) <= tolerance
        observed_counts[observed_indices] += 1
        consistent_counts[observed_indices[consistent]] += 1
        projection_tests += int(observed_indices.size)

    observed = observed_counts > 0
    consistent = consistent_counts >= int(min_consistent_views)
    keep = consistent | (~observed if keep_unobserved else False)
    stats = {
        "input_points": int(points.shape[0]),
        "observed_points": int(np.count_nonzero(observed)),
        "consistent_points": int(np.count_nonzero(consistent)),
        "rejected_points": int(points.shape[0] - np.count_nonzero(keep)),
        "projection_tests": int(projection_tests),
    }
...
def make_depth_points_for_frame(
    ...
    sampled_after_filter = int(colors.shape[0])
    if per_frame_point_samples > 0 and colors.shape[0] > per_frame_point_samples:
        rng = np.random.default_rng(int(seed) + int(frame.index) * 1009)
        keep_indices = rng.choice(colors.shape[0], size=int(per_frame_point_samples), replace=False)
        world = world[keep_indices]
    ...
    return world, colors, {
        ...
        "sampled_after_filter": sampled_after_filter,
        "per_frame_sampled": int(colors.shape[0]),
    }
...
def load_scanapp_scene(
    ...
    target_points: int,
    seed: int,
    keyframe_filter_enabled: bool,
    keyframe_min_translation: float,
    keyframe_window: int,
    keyframe_sharpness_stride: int,
    keyframe_min_frames: int,
    min_motion_quality: float,
    shared_intrinsics_mode: str,
    per_frame_point_samples: int,
    consistency_filter_enabled: bool,
    consistency_neighbor_window: int,
    consistency_min_views: int,
    ...
    if keyframe_filter_enabled:
        frames, keyframe_filter = select_information_gated_keyframes(
            frames,
            keyframe_min_translation,
            keyframe_window,
            keyframe_sharpness_stride,
            keyframe_min_frames,
        )
    else:
        keyframe_filter = {
    ...
    totals = {
        ...
        "colorized": 0,
        "sampled_after_filter": 0,
        "per_frame_sampled": 0,
    }
    consistency_totals = {
        "enabled": bool(consistency_filter_enabled),
        "neighbor_window": int(consistency_neighbor_window),
        "min_consistent_views": int(consistency_min_views),
        "abs_depth_tolerance_meters": float(consistency_abs_depth_tol),
        "rel_depth_tolerance": float(consistency_rel_depth_tol),
        "keep_unobserved": bool(consistency_keep_unobserved),
        "input_points": 0,
        "observed_points": 0,
        "consistent_points": 0,
        "rejected_points": 0,
    ...
    for frame_index, frame in enumerate(frames):
        ...
        points, colors, stats = make_depth_points_for_frame(
            frame,
            sample_step,
            shared_intrinsics,
        ...
        if consistency_filter_enabled and points.size:
            keep, consistency_stats = depth_consistency_keep_mask(
                ...
                cameras,
                depth_cache,
                consistency_neighbor_window,
                consistency_min_views,
                consistency_abs_depth_tol,
                consistency_rel_depth_tol,
                MIN_CONFIDENCE,
                consistency_keep_unobserved,
            )
            points = points[keep]
            colors = colors[keep]
            for key in ("input_points", "observed_points", "consistent_points", "rejected_points", "projection_tests"):
                consistency_totals[key] += int(consistency_stats[key])
            consistency_totals["frame_summaries"].append(
                {
                    "frame_index": int(frame.index),
                    **{key: int(consistency_stats[key]) for key in ("input_points", "observed_points", "consistent_points", "rejected_points", "projection_tests")},
    ...
    return ScanAppScene(
        ...
        keyframe_filter=keyframe_filter,
        consistency_filter=consistency_totals,
        splatking_like={
            "motion_quality_filter": motion_quality_filter,
            "shared_intrinsics_mode": shared_intrinsics_mode,
            "shared_intrinsics": shared_intrinsics.astype(float).tolist() if shared_intrinsics is not None else None,
            "per_frame_point_samples": int(per_frame_point_samples),
...
def init_sh_model_from_points(
    ...
    if quats_wxyz is None:
        ...
        quats = mx.array(quats_wxyz[None, ...], dtype=mx.float32)
...
def normals_to_quats_wxyz(normals: np.ndarray) -> np.ndarray:
    normals = np.asarray(normals, dtype=np.float32)
    normals = normals / np.maximum(np.linalg.norm(normals, axis=1, keepdims=True), 1.0e-8)
    z_axis = np.array([0.0, 0.0, 1.0], dtype=np.float32)
    dots = np.clip(normals @ z_axis, -1.0, 1.0)
    cross = np.cross(np.broadcast_to(z_axis, normals.shape), normals)
    quats = np.empty((normals.shape[0], 4), dtype=np.float32)
    regular = dots > -0.999999
    quats[regular, 0] = 1.0 + dots[regular]
    quats[regular, 1:] = cross[regular]
    quats[~regular] = np.array([0.0, 1.0, 0.0, 0.0], dtype=np.float32)
    quats /= np.maximum(np.linalg.norm(quats, axis=1, keepdims=True), 1.0e-8)
    return quats.astype(np.float32)


def disc_prior_scales_and_quats(
    points: np.ndarray,
    ...
    if points.ndim != 2 or points.shape[1] != 3 or points.shape[0] == 0:
        raise ValueError("points must have shape [N, 3] and be nonempty")
    if points.shape[0] < max(4, normal_knn + 1, tangent_knn + 1):
        log_scales = knn_log_scales_from_points(points, tangent_scale_multiplier)
        quats = normals_to_quats_wxyz(np.tile(np.array([[0.0, 0.0, 1.0]], dtype=np.float32), (points.shape[0], 1)))
        return log_scales, quats, {"fallback": "too_few_points"}
    try:
        from scipy.spatial import cKDTree
    except ImportError as exc:
        raise ImportError("Disc prior initialization requires scipy.spatial.cKDTree.") from exc

    k = max(int(normal_knn), int(tangent_knn)) + 1
    distances, indices = cKDTree(points).query(points, k=k)
    normal_indices = indices[:, 1 : int(normal_knn) + 1]
    neighborhoods = points[normal_indices]
    centered = neighborhoods - neighborhoods.mean(axis=1, keepdims=True)
    cov = np.einsum("nki,nkj->nij", centered, centered) / float(max(1, normal_knn))
    eigenvalues, eigenvectors = np.linalg.eigh(cov)
    normals = eigenvectors[:, :, 0].astype(np.float32)

    tangent_distances = distances[:, 1 : int(tangent_knn) + 1]
    tangent_scale = np.mean(tangent_distances, axis=1).astype(np.float32) * float(tangent_scale_multiplier)
    tangent_scale = np.maximum(tangent_scale, 1.0e-8)
    normal_scale = np.maximum(tangent_scale * float(normal_scale_ratio), 1.0e-8)
    scales = np.stack([tangent_scale, tangent_scale, normal_scale], axis=1).astype(np.float32)
    log_scales = np.log(scales).astype(np.float32)
    quats = normals_to_quats_wxyz(normals)
    stats = {
        "fallback": None,
        "normal_knn": int(normal_knn),
        "tangent_knn": int(tangent_knn),
        "tangent_scale_multiplier": float(tangent_scale_multiplier),
        "normal_scale_ratio": float(normal_scale_ratio),
        "tangent_scale_min": float(tangent_scale.min()),
        "tangent_scale_mean": float(tangent_scale.mean()),
        "tangent_scale_max": float(tangent_scale.max()),
        "normal_scale_min": float(normal_scale.min()),
        "normal_scale_mean": float(normal_scale.mean()),
        "normal_scale_max": float(normal_scale.max()),
        "normal_eigenvalue_mean": np.mean(eigenvalues, axis=0).astype(float).tolist(),
    }
    return log_scales, quats, stats
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/scanapp_depth_consistency_train"))
    ...
    parser.add_argument("--keyframe-filter-enabled", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--keyframe-min-translation", type=float, default=0.05)
    parser.add_argument("--keyframe-window", type=int, default=8)
    parser.add_argument("--keyframe-sharpness-stride", type=int, default=8)
    ...
    parser.add_argument("--consistency-min-views", type=int, default=1)
    parser.add_argument("--consistency-abs-depth-tol", type=float, default=0.08)
    parser.add_argument("--consistency-rel-depth-tol", type=float, default=0.03)
    ...
    parser.add_argument("--summary-dataset-type", type=str, default="scanapp_depth_consistency_mobile_prior_masked")
    parser.add_argument("--summary-experiment", type=str, default=None)
...
def main() -> None:
    ...
    if args.keyframe_min_translation < 0.0:
        raise ValueError("--keyframe-min-translation must be nonnegative")
    if args.keyframe_window <= 0:
        raise ValueError("--keyframe-window must be positive")
    if args.keyframe_sharpness_stride <= 0:
        raise ValueError("--keyframe-sharpness-stride must be positive")
    if args.keyframe_min_frames <= 0:
        raise ValueError("--keyframe-min-frames must be positive")
    if args.min_motion_quality < 0.0:
    ...
    if args.prior_tangent_knn < 1:
        raise ValueError("--prior-tangent-knn must be positive")
    if args.consistency_neighbor_window <= 0:
        raise ValueError("--consistency-neighbor-window must be positive")
    if args.consistency_min_views <= 0:
        raise ValueError("--consistency-min-views must be positive")
    if args.consistency_abs_depth_tol < 0.0:
        raise ValueError("--consistency-abs-depth-tol must be nonnegative")
    if args.consistency_rel_depth_tol < 0.0:
        raise ValueError("--consistency-rel-depth-tol must be nonnegative")
    ...
    for name, value in [
        ...
        ("--prior-normal-scale-ratio", args.prior_normal_scale_ratio),
        ("--prior-tangent-scale-multiplier", args.prior_tangent_scale_multiplier),
    ...
    scene = load_scanapp_scene(
        ...
        seed=args.seed,
        keyframe_filter_enabled=args.keyframe_filter_enabled,
        keyframe_min_translation=args.keyframe_min_translation,
        keyframe_window=args.keyframe_window,
        keyframe_sharpness_stride=args.keyframe_sharpness_stride,
        keyframe_min_frames=args.keyframe_min_frames,
        min_motion_quality=args.min_motion_quality,
        shared_intrinsics_mode=args.shared_intrinsics,
        per_frame_point_samples=args.per_frame_point_samples,
        consistency_filter_enabled=args.consistency_filter_enabled,
        consistency_neighbor_window=args.consistency_neighbor_window,
        consistency_min_views=args.consistency_min_views,
    ...
    if args.eval_max_frames > 0:
        eval_frames = select_frames(load_scanapp_frames(args.data), args.eval_max_frames, eval_frame_step, args.eval_start_index)
        if args.keyframe_filter_enabled:
            eval_frames, _ = select_information_gated_keyframes(
                eval_frames,
                args.keyframe_min_translation,
                args.keyframe_window,
                args.keyframe_sharpness_stride,
    ...
    log(
        "initializing mobile-prior Gaussian parameters "
        f"points={points.shape[0]} raw_depth_points={raw_point_count} "
        f"target_points={args.target_points} prior_init_mode={args.prior_init_mode}"
    )
    init_quats = None
    prior_init_stats = {"mode": str(args.prior_init_mode)}
    if args.prior_init_mode == "disc":
        log_scales, init_quats, disc_stats = disc_prior_scales_and_quats(
            points,
            args.prior_tangent_scale_multiplier,
            args.prior_normal_scale_ratio,
            args.prior_normal_knn,
            args.prior_tangent_knn,
        )
        prior_init_stats.update(disc_stats)
        scale_rule = "PocketGS-style disc-like covariance seeding from KNN normals"
    else:
    ...
    model = init_sh_model_from_points(points, colors, log_scales, args.opacity, args.sh_degree, init_quats)
    ...
    summary = {
        "dataset_type": str(args.summary_dataset_type),
        "experiment": args.summary_experiment,
        ...
        "initialization": {
            "type": "scanapp_depth_mobile_prior_reconstruction" if args.prior_init_mode == "disc" else "scanapp_depth_knn_reconstruction",
            "scale_rule": scale_rule,
            "sampling_rule": (
                "valid depth pixels are reconstructed, capped per frame, then globally sampled to target_point_count"
                if args.per_frame_point_samples > 0
                else "all valid depth pixels are reconstructed first, then globally sampled to target_point_count"
            ),
            "prior_init": prior_init_stats,
        ...
        "paper_mapping": {
            "PocketGS": {
                "implemented": [
                    "information-gated frame subsampling using displacement, sharpness, and windowed selection",
                    "cross-view depth consistency filtering before Gaussian initialization",
                    "local-surface Gaussian seeding with KNN normals, tangent scales, compressed normal scales, and normal-aligned rotations",
                ],
                "not_implemented": [
                    "global bundle adjustment",
                    "iterative re-triangulation",
                    "single-reference MVS densification",
                    "hardware-aligned renderer changes",
                ],
            },
            "LighthouseGS": {
                "implemented": [
                    "keeps ARKit/ScanApp poses and depth-derived geometry as mobile priors",
                ],
                "not_implemented": [
                    "plane scaffold assembly",
                    "stable pruning for low-texture planes",
    ...
    log(
        "ScanApp consistency-filtered mobile-prior masked depth multi-view training ok "
